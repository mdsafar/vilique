...
begin
  ...
  -- Always validate required fields for every update (draft and published)
  if btrim(coalesce(p_patch->>'title', invite.title, '')) = '' then
    required_errors := required_errors || jsonb_build_object('title', 'Title is required.');
  end if;
  if btrim(coalesce(p_patch->>'primaryName', invite.primary_name, '')) = '' then
    required_errors := required_errors || jsonb_build_object('primaryName', 'Primary name is required.');
  end if;
  if btrim(coalesce(p_patch->>'secondaryName', invite.secondary_name, '')) = '' then
    required_errors := required_errors || jsonb_build_object('secondaryName', 'Secondary name is required.');
  end if;
  if btrim(coalesce(p_patch->>'eventDate', invite.event_date::text, '')) = '' then
    required_errors := required_errors || jsonb_build_object('eventDate', 'Event date is required.');
  end if;
  if btrim(coalesce(p_patch->>'eventTime', invite.event_time, '')) = '' then
    required_errors := required_errors || jsonb_build_object('eventTime', 'Event time is required.');
  end if;
  if btrim(coalesce(p_patch->>'venueName', invite.venue_name, '')) = '' then
    required_errors := required_errors || jsonb_build_object('venueName', 'Venue name is required.');
  end if;
  if btrim(coalesce(p_patch->>'venueAddress', invite.venue_address, '')) = '' then
    required_errors := required_errors || jsonb_build_object('venueAddress', 'Address is required.');
  end if;
  if btrim(coalesce(p_patch->>'phone', invite.phone, '')) = '' then
    required_errors := required_errors || jsonb_build_object('phone', 'Primary phone is required.');
  elsif length(coalesce(p_patch->>'phone', invite.phone)) <> 10 then
    required_errors := required_errors || jsonb_build_object('phone', 'Primary phone must be 10 digits.');
  end if;
  if btrim(coalesce(p_patch->>'secondaryPhone', invite.secondary_phone, '')) = '' then
    required_errors := required_errors || jsonb_build_object('secondaryPhone', 'Secondary phone is required.');
  elsif length(coalesce(p_patch->>'secondaryPhone', invite.secondary_phone)) <> 10 then
    required_errors := required_errors || jsonb_build_object('secondaryPhone', 'Secondary phone must be 10 digits.');
  end if;
  if btrim(coalesce(p_patch->>'message', invite.message, '')) = '' then
    required_errors := required_errors || jsonb_build_object('message', 'Message is required.');
  end if;
